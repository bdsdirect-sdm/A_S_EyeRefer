...
interface AddAddressProps {
  onAdd: (newAddress: string) => void;  // onAdd function that receives the new address
}

const AddAddress: React.FC<AddAddressProps> = ({ onAdd }) => {
  const navigate = useNavigate();
  ...
  const addressMutation = useMutation({
    ...
    onSuccess: (data) => {
      toast.success("Address Saved");
      // Here, pass the new address data to the parent via the onAdd callback
      onAdd(data.address); // Assuming the response has a property called 'address'
      navigate("/profile");
    },
  });

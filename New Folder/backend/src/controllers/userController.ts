...
export const deleteAddress = async (req: any, res: any) => {
    try {
        const { uuid } = req.user;  // Assuming `uuid` is available in `req.user` (from authentication middleware)
        console.log("<><<<<<<<<<<", uuid);

        const { addressUuid } = req.params;  // `addressUuid` instead of `addressId`
        ...
        // Find and delete the address by the provided `addressUuid`
...
export const addStaff = async (req: any, res: any) => {
    try {
        ...
        const { uuid } = req.user;  // Optional: if you want to associate the staff with the logged-in user
        ...
        // Check if the staff already exists based on email (optional)
        ...
        const staff = await Staff.create({
            ...
            contact,
            gender,
            createdBy: uuid,  // Optional: associate with the user who created the staff
        });
